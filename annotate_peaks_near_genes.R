...
# select peaks that are NOT at promoters
peaks_not_promoter = rbind(annotatedPeak[which(annotatedPeak$insideFeature ==
                                                "downstream"), ],
                           annotatedPeak[which(annotatedPeak$insideFeature ==
                                                 "overlapEnd"), ],
                          annotatedPeak[which(annotatedPeak$insideFeature ==
...
# they are still assigned to a close gene by proximity
...
all_ensembl_info_ensemblFound <-
  getBM(
    attributes = c('ensembl_gene_id', 'external_gene_name', 'gene_biotype'),
    filters = 'ensembl_gene_id',
    values = peaks_not_promoter$feature,
    mart = ensembl
  )

# change column name for emsebl gene ids to merge with new df
colnames(peaks_not_promoter)[names(peaks_not_promoter) == "feature"] = "ensembl_gene_id"
peaks_not_promoter = merge( all_ensembl_info_ensemblFound, peaks_not_promoter,by =
                             "ensembl_gene_id") # merge old and new info


peaks_not_promoter = peaks_not_promoter[which(peaks_not_promoter$gene_biotype %in% c("protein_coding", "lincRNA")), ]
...
write.table(
  ...
  file = paste(
    "/Users/Marta/Documents/WTCHG/DPhil/Data/Regulation/atac-seq/",
    currentDate,
    "annotated_peaks_nearby_non_promoters_allCPMs.txt",
    sep = ""
  ),
  sep = "\t",
  col.names = T,
...
write.table(
  ...
  file = paste(
    "/Users/Marta/Documents/WTCHG/DPhil/Data/Regulation/atac-seq/",
    currentDate,
    "annotated_peaks_nearby_non_promoters_over10CPMs.txt",
    sep = ""
  ),
  sep = "\t",
  col.names = T,
