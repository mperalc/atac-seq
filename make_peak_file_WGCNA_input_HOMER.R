...
dir = "/Users/Marta/Documents/WTCHG/DPhil/Data/Results/ATAC-seq/WGCNA/10CPM_P12_S120_deepSplit2_signedHybrid_noOutliers/"
peaks = read_delim(file = paste(dir,"fGWAS_WGCNA_ATAC_pval_selected_MM.bed", sep = ""),
                   delim = "\t",
                   col_names = F)

modules_selected = c("royalblue", "lightyellow", "purple", "pink", "blue", "green") # change every time according to significantly enriched modules

############################################################

colnames(peaks) = c("Chr", "Start", "End", "Module")
peaks$Name = paste("peak", peaks$Chr, peaks$Start, sep = "_")

p = list()
for (s in modules_selected) {
  p[[s]] = peaks[which(peaks$Module == s), c("Name", "Chr", "Start", "End")]
